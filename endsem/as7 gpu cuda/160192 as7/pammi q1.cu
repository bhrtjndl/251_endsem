...
{
      int thread_id = threadIdx.x + rows * threadIdx.y;//flattened thread id value;;
      int i = blockDim.x * blockDim.y * blockIdx.x + thread_id;
//      int i_x = blockDim.x * blockIdx.x + threadIdx.x;
//      int i_y = blockDim.x * blockIdx.x + threadIdx.y;
//      int i = i_x + rows * i_y;
//     printf("%d %d %d\n", i, blockDim.y, blockIdx.y);
      //why blockDim.y is not zero??
...
int main(int argc, char **argv)
{
    ...
    int rows, cols;   /*Default value of num from MACRO*/
    int blocks;
    ...
    calculate<<<blocks, threadsPerBlock >>>(gpu_mem, num, rows);
